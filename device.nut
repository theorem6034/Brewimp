...
hardware.pin3.configure(DIGITAL_OUT); //chip select
...
function readStrike189(){
        //Get SPI data 
    hardware.pin3.write(0); //pull CS low to start the transmission of temp data  
      //0[31..24],1[23..16],2[15..8],3[7..0]
        temp32=hardware.spi189.readblob(4);//SPI read is totally completed here
    hardware.pin3.write(1); // pull CS high
        // Begin converting Binary data for chip 1
    local tc = 0;
    if ((temp32[1] & 1) ==1){
        //Error bit is set
...
hardware.pin3.write(1); //Set the Chip Select pin to HIGH prior to SPI read
